...
const result = await new Promise((resolve) => {
  const child = spawn(process.execPath, [path.join(root, 'scripts/install-preset.mjs'), path.join(target, 'research'), '--clean-target'], { cwd: root, stdio: ['ignore', 'pipe', 'pipe'] })
...
assert.equal(receipt.replaceConfig, false)
